...
const gratisanTextPath = path.join(__dirname, 'gratisan_text.json');
const defaultGratisanText = { text: 'Belum ada info akun gratisan dari admin.' };

function loadGratisanText() {
  try { return JSON.parse(fs.readFileSync(gratisanTextPath, 'utf8')); }
  catch (e) { return defaultGratisanText; }
}
function saveGratisanText(text) {
  const payload = { text };
  fs.writeFileSync(gratisanTextPath, JSON.stringify(payload, null, 2), 'utf8');
  return payload;
}
...
async function sendMainMenu(ctx) {
  ...
  const messageText = `
<code>┏━━━━━━━━━━━━━━━━━━━━┓</code>
<b>🚀 SELAMAT DATANG DI VPN ${NAMA_STORE}</b>
<code>┗━━━━━━━━━━━━━━━━━━━━┛</code>
🤖 <b>Kegunaan Bot:</b>
Bot ini menyediakan layanan pembuatan akun VPN Premium (SSH, Vmess, Vless, Trojan, ZIVPN, UDP Custom) secara instan, aman, dan otomatis. Anda bisa langsung melakukan deposit dan membuat akun 24/7!
...
`;
  ...
  let keyboard = [
    [
      { text: '💎 Buat Akun Premium', callback_data: 'service_create' },
      { text: '🆓 Buat Akun Gratisan', callback_data: 'menu_gratisan' }
    ],
    [
...
bot.action('menu_gratisan', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    
    // Ambil server yang harga = 0 (Gratisan)
    db.all('SELECT id, nama_server FROM Server WHERE harga = 0', [], (err, servers) => {
        if (err || servers.length === 0) {
            return ctx.reply('❌ Maaf, server gratisan sedang tidak tersedia.');
        }

        const buttons = servers.map(s => [{
            text: `✅ ${s.nama_server}`,
            callback_data: `select_server_${s.id}_free` // Flag 'free' ditangkap di action selanjutnya
        }]);

        ctx.reply('Pilih server untuk akun gratisan (Maks 3 Hari / 2 IP):', {
            reply_markup: { inline_keyboard: [...buttons, [{ text: '❌ Batal', callback_data: 'send_main_menu' }]] }
        });
    });
});
// Contoh jika Anda punya handler server yang umum
bot.action(/select_server_(\d+)_?(.*)/, async (ctx) => {
    const serverId = ctx.match[1];
    const isFree = ctx.match[2] === 'free';
    
    // Simpan ke state agar saat proses create bisa dibedakan
    userState[ctx.chat.id] = { serverId, isFree, step: 'service_selection' }; 
    // ... arahkan ke pemilihan tipe akun (vmess, ssh, dll)
});
bot.action('pay_via_saldo', async (ctx) => {
    const state = userState[ctx.chat.id];
    if (state.isFree) {
        // Lompati proses potong saldo
        // Gunakan parameter: exp = 3, iplimit = 2
        // ... (panggil fungsi createvmess/createvless/dll dengan parameter ini)
    } else {
        // ... (jalankan kode pemotongan saldo seperti biasa)
    }
});
...
bot.action('topup_saldo', async (ctx) => {
  try {
    ...
    await ctx.editMessageText(
      ...
      '• Biaya layanan bot **RANDOM 200-500**\n' +
...
bot.action('jadi_reseller', async (ctx) => {
  ...
  const inlineKeyboard = [
    [{ text: '📞 Hubungi Admin', url: waUrl || 'https://t.me/' + getAdminTelegramUsername().replace('@','') }],
    [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
  ];
  await ctx.reply(
    '*🤝 PROGRAM RESELLER*\n\n' +
    'Untuk menjadi reseller dan mendapatkan harga khusus, silakan hubungi admin langsung melalui tombol di bawah ini.',
...
if (!state || !state.step) return;
  ...
  if (state.step === 'maintenance_estimate_input') {
    ...
    bot.action('pay_via_qris', async (ctx) => {
   await ctx.answerCbQuery().catch(()=>{});
   const state = userState[ctx.chat.id];
   if(!state || !state.totalHarga) return ctx.reply('Sesi expired. Silakan ulangi pembelian dari menu utama.');
   ...
   global.depositState[userId] = {
     ...
     amount: totalHarga.toString(),
     topupPurpose: 'regular',
     minAmount: getMinTopupByGatewayMode(PAYMENT_GATEWAY_MODE)
   };
   
   await ctx.reply(`⏳ Mengarahkan ke pembayaran QRIS untuk Rp ${totalHarga.toLocaleString('id-ID')}...`);
   await processDeposit(ctx, totalHarga, global.depositState[userId]);
   delete userState[ctx.chat.id];
});
...
bot.action('pay_via_saldo', async (ctx) => {
    ...
    const { username, password, exp, quota, iplimit, serverId, type, action, totalHarga, selectedPackage } = state;
    let usedPassword = password || '';
    let msg;

    db.get('SELECT * FROM Server WHERE id = ?', [serverId], async (err, server) => {
        if (err || !server) return ctx.reply('❌ *Server tidak ditemukan.*', { parse_mode: 'Markdown' });

        db.get('SELECT saldo FROM users WHERE user_id = ?', [ctx.from.id], async (err, user) => {
            if (err || !user) return ctx.reply('❌ *Pengguna tidak ditemukan.*', { parse_mode: 'Markdown' });

            if (user.saldo < totalHarga) {
                return ctx.reply('❌ *Saldo Anda tidak mencukupi untuk melakukan transaksi ini. Silakan Top Up terlebih dahulu.*', { parse_mode: 'Markdown' });
            }
            ...
            if (!reserveResult.ok) {
                ...
                return ctx.reply('❌ *Terjadi kesalahan saat memproses saldo.*', { parse_mode: 'Markdown' });
            }
            ...
            if (action === 'create') {
                ...
                if (type === 'vmess') msg = await createvmess(username, exp, quota, iplimit, serverId, telegramUserId, telegramChatId);
                    ...
                    usedPassword = Math.random().toString(36).slice(-8);
                    msg = await createzivpn(username, usedPassword, exp, iplimit, serverId, telegramUserId, telegramChatId, selectedPackage);
                }
                ...
                else if (type === 'udp_http') msg = await renewudphttp(username, exp, iplimit, serverId);
                else if (type === 'zivpn') msg = await renewzivpn(username, exp, iplimit, serverId);
            }

            const msgText = String(msg || '');
            const msgLower = msgText.toLowerCase();
            const isDuplicateUsername = action === 'create' && (msgLower.includes('username sudah ada') || msgLower.includes('username already exists') || msgLower.includes('exists'));

            if (isDuplicateUsername) {
                await cancelReservedAccountCharge(ctx.from.id, totalHarga, reserveResult.referenceId);
                state.step = `username_${action}_${type}`;
                delete state.username; delete state.exp; delete state.totalHarga;
                return ctx.reply('Username yang kamu masukkan sudah dipakai di server.\nSilakan gunakan username lain yang unik.\n\nMasukkan username baru:', { parse_mode: 'Markdown' });
            }

            const isErrorMsg = ['error', 'gagal', 'failed', 'not found', 'tidak ditemukan'].some((needle) => msgLower.includes(needle));
            const isRenewSuccessMsg = action === 'renew' && (msgLower.includes('berhasil diperpanjang') || msgLower.includes('success') || msgLower.includes('premium'));
            ...
            await finalizeReservedAccountCharge(reserveResult.referenceId, type);

            if (action === 'create') {
                db.run('UPDATE Server SET total_create_akun = total_create_akun + 1 WHERE id = ?', [serverId]);
            }

            let expiresAt = exp > 0 ? (Date.now() + exp * 24 * 60 * 60 * 1000) : null;
            if (action === 'renew' && exp > 0) {
                const existingExpiry = await getAccountExistingExpiry(ctx.from.id, type, username, serverId, state.serverDomain);
                expiresAt = Math.max(Date.now(), Number(existingExpiry || 0)) + exp * 24 * 60 * 60 * 1000;
            }

            const passwordToStore = (type === 'zivpn') ? usedPassword : password;
            const linkPayload = (['vmess', 'vless', 'trojan'].includes(type)) ? extractAccountLinksFromMessage(msgText) : {};
            
            upsertAccountRecord({
                userId: ctx.from.id, type, username, password: passwordToStore,
                serverId, serverName: state.serverName, domain: state.serverDomain,
                accountIpPackage: selectedPackage, accountPricePerDay: (totalHarga / exp), expiresAt, ...linkPayload
            });
            ...
            if (action === 'create') {
                try {
                    ...
                    await sendGlobalCreateAccountNotification({
                        creatorId: ctx.from.id, creatorUsername,
                        serverName: state.serverName || state.serverDomain || '-',
                        accountType: type.toUpperCase(), role: isReseller ? 'RESELLER' : 'USER',
                        remarks: buildCreateNotifRemarks(type, username), expDays: exp,
                        expiredDate: formatDateId(expDate), payment: `VIA SALDO BOT - Rp ${totalHarga.toLocaleString('id-ID')}`
                    });
                } catch (e) { }
            }

            const msgToUser = action === 'create' ? normalizeCreateAccountMessageForDisplay(msg, selectedPackage) : msg;
...
if (action === 'trial') {
  ...
  if (state.step.startsWith('username_')) {
    ...
    if (isNaN(exp) || exp < 7) {
      return ctx.reply('❌ *Masa aktif minimal 7 hari.*', { parse_mode: 'Markdown' });
    }
    if (exp > 365) {
    ...
    db.get('SELECT quota, iplimit, domain, nama_server FROM Server WHERE id = ?', [state.serverId], async (err, server) => {
      ...
      if (state.action === 'renew' && Number(state.accountIpLimit) > 0) {
        ...
        try { state.iplimit = await getServerIpLimitRule(server.id, state.type, selectedPkg); }
        catch (limitErr) { state.iplimit = getDefaultServerIpLimit(state.type, selectedPkg); }
      } else {
      ...
      const selectedPackage = (() => {
        if (state.action === 'renew') {
          const renewPkg = Number(state.accountIpPackage || state.selectedIpPackage || inferIpPackageByAccount(state.type, state.accountIpLimit || state.iplimit || server.iplimit));
      ...
      const paymentKeyboard = [
        [{ text: `💳 Bayar Pakai Saldo (Rp ${totalHarga.toLocaleString('id-ID')})`, callback_data: 'pay_via_saldo' }],
        [{ text: `📲 Rekomendasi QRIS Langsung`, callback_data: 'pay_via_qris' }],
      ...
      await ctx.reply(
        ...
        { parse_mode: 'Markdown', reply_markup: { inline_keyboard: paymentKeyboard } }
      );
    });
